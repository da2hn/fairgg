	...
	private void parse() {
		...
		try {
			...
			if (versionEndIndex < 0) {
				...
				agentSubstring = StringUtils.substring(userAgentString, versionStartIndex + 1);
			} else {
				//agentSubstring = userAgentString.substring(versionStartIndex + 1, versionEndIndex);
				agentSubstring = StringUtils.substring(userAgentString, versionStartIndex + 1, versionEndIndex);
			}
			browserVersion = toFloat(agentSubstring);
		} catch (NumberFormatException e) {
			// Just use the default value.
			browserVersion = (float) 1.0;
		}
