...
$(document).ready(function() {
	...
	$("#btnDropMob").click(function(){
		location.href = "/myPage/expr/frnchsInfo/frnchsInfoList.do";
	});
	$("#btnModifyMob").click(function(){
		fn_mobModifyFrnchsInfo()
	});

	$("input[type=file][name=frnchsImageFile]").on("change", function() {
		readURL(this);
	});
...
function fn_init() {
	...
	fObj = new fileObj({objId:"f1", windowMode:"full", divId:$("#atchFileDiv"), readOnly:false, addCnt:"M3", filePath:"basic", maxFileSize:"5", fileType:"normal", tmpDel:bEntrprsIntrcnFileNo});
	fObj.init();
...
function fn_modifyFrnchsInfo() {
	if (fn_isValid()) {
		// 첨부파일
		$('#entrprsIntrcnFileNo').val(fObj.getatchmnflNo());
...
function fn_mobModifyFrnchsInfo() {
	if (fn_mobIsValid()) {
		// 첨부파일
		$('#mEntrprsIntrcnFileNo').val(m_fObj.getatchmnflNo());
...
function fn_isValid() {
	if(!$.trim($("[name=frnchsInfo]").val())) {
		alert("프랜차이즈 정보을 입력해주세요.");
		$("[name=frnchsInfo]").focus();
		return false;
	}
	if(!$("#imgY").val() && !$("#fileName").val()) {
		alert("프랜차이즈 이미지를 입력해주세요.");
		return false;
	}
	return true;
}
...
function fn_refresh() {
	$("#dataForm").attr("action","/myPage/expr/frnchsInfo/frnchsInfo.do");
	$("#dataForm").submit();
}