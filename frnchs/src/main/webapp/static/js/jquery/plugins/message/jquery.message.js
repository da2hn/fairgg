	...
	jQuery.extend({
		...
		showAlert: function(msg, parameters) {
			...
			if ($(".progressBarDiv").length > 0 ) {
				...
				$(".progressBarDiv").find(".progress-text1").text("데이터를 불러오는중입니다.");
			}
		...
		showChoose: function(msg, parameters) {
			...
			$.alerts.okButton = parameters.okButtonText;
			$.alerts.cancelButton = parameters.cancelButtonText;
			if (parameters) {
				map = jQuery.extend(map,parameters);	//변수에 담긴값 셋팅
		...
		$.alerts = {
			...
			_show: function(map, msg, value, type, callback) {
				...
				switch( type ) {
					case 'alert':
						if (map.autoHide) {
							...
							*/
							...
							$("#popup_message").after('<div id="popup_panel"><input type="button" value="' + $.alerts.okButton + '" id="popup_ok" class="btn btnGreen" /></div>');
					...
					break;
					case 'confirm':
						$("#popup_message").after('<div id="popup_panel"><input type="button" value="' + $.alerts.okButton + '" id="popup_ok" class="btn btnGreen" /> <input type="button" value="' + $.alerts.cancelButton + '" id="popup_cancel" class="btn btnLgray" /></div>');
						$("#popup_ok").click( function() {
							$.alerts._hide();
				...
				//버튼 롤오버 효과
				$("#popup_container").find(".btn").bind("mouseover", function(){
					$(this).attr("data-color", $(this).css("color")).attr("data-bg", $(this).css("background-color"));
					$(this).stop().animate({ "color":"#fff", "background-color": "#000" },100);
				});
				$("#popup_container").find(".btn").bind("mouseout", function(){
					$(this).stop().animate({ "color":$(this).data("color"), "background-color": $(this).data("bg") },100);
				});
			},
