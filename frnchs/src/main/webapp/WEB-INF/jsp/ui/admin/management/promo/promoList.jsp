...
%>
	...
	function drawTable(data,area){
		...
		if( data.length < 1 ){
			...
			tmpHtml.push('<tr><td colspan="7">데이터 없음</td></tr>');
			...
			data.forEach(function(row,idx){
				...
				tmpHtml.push('		<a href="javascript:void(0)" id="'+row.recordId+'" class="ul detailBtn">'+row.promoFcName+'</a>');
				tmpHtml.push('	</td>');
				tmpHtml.push('	<td>');
				tmpHtml.push(		row.useYn);
				tmpHtml.push('	</td>');
				tmpHtml.push('	<td>');
				tmpHtml.push(		row.promoStartDate);
				tmpHtml.push('	</td>');
				tmpHtml.push('	<td>');
				tmpHtml.push(		row.promoEndDate);
				tmpHtml.push('	</td>');
				tmpHtml.push('	<td>');
		...
		$(".detailBtn").off("click").on("click",function(e){
			...
			location.href ="${contextPath}/admin/management/promo/promoDetail.do?recordId="+$(this).attr("id");
		});
	...
	$(document).ready(function(){
		...
		var data ={};
		cmAjax(
			"${contextPath}/admin/management/promo/searchAjax.do",				//url
			data,																//data
		...
		$("#searchBtn").off("click").on("click",function(e){
			...
			var data2 ={};
			//data2['popupTitle']=$("#popupTitle").val();
		...
		$("#regBtn").off("click").on("click",function(e){
			e.preventDefault();
			location.href = "${contextPath}/admin/management/promo/promoDetail.do";
		})
		...
		$("#delBtn").off("click").on("click",function(e){
			...
			cmAjax(
					"${contextPath}/admin/management/promo/delete.do",		//url
				...
				<div class="co">
					<span class="gSelect">
						<select title="팝업제목" class="select" name="searchItem" id="searchItem">
							<option value="promoTitle">홍보영상 제목</option>
						</select>
					</span>
	...
	<div class="mButton1">
		...
		<span class="gRight">
			<a href="javascript:void(0)" id="regBtn" class="mBtn1 blue regBtn">동영상 등록</a>
		</span>
	</div>
